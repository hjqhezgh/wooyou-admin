...
func TmkStatisticsAction(w http.ResponseWriter, r *http.Request) {
	...
	sql += " left join "
	sql += " (select count(1) num,create_user from schedule_detail_child where create_time>=? and create_time <=? and wyclass_id is not null and (contract_id=0 or contract_id is null)	group by create_user) cc on em.user_id=cc.create_user "
	sql += " left join "
	...
	sql += " where si.type=1 and (si.card_id is null or si.card_id=0 or si.card_id=00000000) and wyclass_id is not null and sign_time>=? and sign_time<=? ) a group by tmk_id )dd on em.user_id=dd.tmk_id "
	sql += " left join "
