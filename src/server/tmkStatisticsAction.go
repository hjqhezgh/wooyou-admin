...
func TmkStatisticsAction(w http.ResponseWriter, r *http.Request) {
	...
	sql += " where si.type=1 and (si.card_id is null or si.card_id=0 or si.card_id=00000000) and sign_time>=? and sign_time<=? ) a group by tmk_id )dd on em.user_id=dd.tmk_id "
	sql += " left join "
