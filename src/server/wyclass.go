...
//学员手动签到
func ClassPayAction(w http.ResponseWriter, r *http.Request) {
	...
	getConsumerIdSql := "select cons.id from child ch left join consumer_new cons on cons.parent_id=ch.pid where ch.cid=?"
	lessgo.Log.Debug(getConsumerIdSql)

	rows, err := db.Query(getConsumerIdSql, childId)
	...
	insertPayLogSql := "insert into pay_log(consumer_id,pay_time,employee_id) values(?,?,?)"
	lessgo.Log.Debug(insertPayLogSql)

	stmt, err := tx.Prepare(insertPayLogSql)
	if err != nil {
		tx.Rollback()
	...
	_, err = stmt.Exec(consumerId,time.Now().Format("20060102150405"), employee.UserId)
	if err != nil {
		tx.Rollback()
	...
	updateConsumerSql := "update consumer_new set pay_status=?, pay_time=? where id=?"
	...
	_, err = stmt.Exec(payType,time.Now().Format("20060102150405"), consumerId)
	if err != nil {
		tx.Rollback()
	...
	checkSignSql := "select count(1) from sign_in where wyclass_id=? and child_id=? "
	lessgo.Log.Debug(checkSignSql)

	rows, err = db.Query(checkSignSql, classId,childId)

	total := 0

	if rows.Next() {

		err = commonlib.PutRecord(rows, &total)
	...
	if total == 0 {
		...
		_, err = stmt.Exec(CONSUMER_STATUS_SIGNIN,time.Now().Format("20060102150405"),consumerId)
		if err != nil {
			tx.Rollback()
